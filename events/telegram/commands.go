...
func (p *Processor) sendList(chatID int, username string) (err error) {
	...
	pages, err := p.storage.RetriewList(context.Background(), username)
	if err != nil && !errors.Is(err, storage.ErrNoSavedPages){
		return err
	}
	...
	// for i := range pages {
    // 	text += strconv.Itoa(i + 1) + "." + "\t" + pages[i].URL + "\n"
	// }
...
// 	p.sendList(chatID, username)
//	p.tg.SendMessage(chatID, msgWhichPageDeleted)